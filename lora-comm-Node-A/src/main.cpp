...
void loop() {
  ...
  if (voltage >= 3.5 && voltage <= 5.0) {
     signal_led("good");
     Serial.println("AT+SEND=4,4,A_on");
  } else if (voltage > 0.9 && voltage < 3.5) {
      signal_led("low");
     Serial.println("AT+SEND=4,5,A_low");
  } else {
    signal_led("down");
    Serial.println("AT+SEND=4,6,A_down");
  }
  delay(5000);  // Delay for stability (you can adjust this as needed)
}